...
HdLighthouse2RenderBuffer::HdLighthouse2RenderBuffer(SdfPath const& id)
    : HdRenderBuffer(id)
    , _width(0)
    , _height(0)
...
HdLighthouse2RenderBuffer::~HdLighthouse2RenderBuffer() = default;

/*virtual*/
void
HdLighthouse2RenderBuffer::Sync(HdSceneDelegate* sceneDelegate,
    ...
    HdDirtyBits* dirtyBits)
{
    if (*dirtyBits & DirtyDescription) {
        ...
        //static_cast<HdLighthouse2RenderParam*>(renderParam)->AcquireSceneForEdit();
    }
...
/*virtual*/
void
HdLighthouse2RenderBuffer::Finalize(HdRenderParam* renderParam)
{
    // Embree has the background thread write directly into render buffers,
...
/*virtual*/
void
HdLighthouse2RenderBuffer::_Deallocate()
{
...
/*static*/
size_t
HdLighthouse2RenderBuffer::_GetBufferSize(GfVec2i const& dims, HdFormat format)
{
...
/*static*/
HdFormat
HdLighthouse2RenderBuffer::_GetSampleFormat(HdFormat format)
{
...
/*virtual*/
bool
HdLighthouse2RenderBuffer::Allocate(GfVec3i const& dimensions,
    HdFormat format,
    bool multiSampled)
{
    _Deallocate();
...
void
HdLighthouse2RenderBuffer::Write(
...
void
HdLighthouse2RenderBuffer::Clear(size_t numComponents, float const* value)
{
    size_t formatSize = HdDataSizeOfFormat(_format);
...
void
HdLighthouse2RenderBuffer::Clear(size_t numComponents, int const* value)
{
    size_t formatSize = HdDataSizeOfFormat(_format);
    for (size_t i = 0; i < _width * _height; ++i) {
        uint8_t* dst = &_buffer[i * formatSize];
...
/*virtual*/
void
HdLighthouse2RenderBuffer::Resolve()
{
